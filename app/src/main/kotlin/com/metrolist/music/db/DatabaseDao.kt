...
interface DatabaseDao {
    ...
    // Song Stats
    @Query("""
        SELECT COUNT(*) as playCount, SUM(playTime) as totalTime
        FROM event
        WHERE songId = :songId
    """)
    fun getSongPlayCount(songId: String): Flow<SongPlayCountResult>

    @Query("""
        SELECT MIN(timestamp) as firstListenDate, MAX(timestamp) as lastListenDate
        FROM event
        WHERE songId = :songId
    """)
    fun getSongListenDates(songId: String): Flow<SongListenDatesResult>

    @Query("""
        SELECT COUNT(*) + 1 as rank
        FROM (
            SELECT songId, SUM(playTime) as totalTime
            FROM event
            GROUP BY songId
            HAVING totalTime > (SELECT SUM(playTime) FROM event WHERE songId = :songId)
        )
    """)
    fun getSongPlayTimeRank(songId: String): Flow<Int>

    @Query("""
        SELECT songId, SUM(playTime) as totalTime, COUNT(*) as playCount
        FROM event
        WHERE songId IN (:songIds)
        GROUP BY songId
    """)
    fun getPlaylistSongStats(songIds: List<String>): Flow<List<PlaylistSongStatResult>>

    @Query("""
        SELECT songId, SUM(playTime) as totalTime, COUNT(*) as playCount
        FROM event
        WHERE songId IN (SELECT songId FROM playlist_song_map WHERE playlistId = :playlistId)
        GROUP BY songId
    """)
    fun getPlaylistSongStatsByPlaylistId(playlistId: String): Flow<List<PlaylistSongStatResult>>
    ...
    @Query("""
        ...
        WHERE songId IN (SELECT songId FROM playlist_song_map WHERE playlistId = :playlistId)
    """)
    fun getPlaylistTotalListenTime(playlistId: String): Flow<Long?>

    @Query("""
        SELECT COUNT(DISTINCT songId) as uniqueSongs, COUNT(*) as totalPlays
        FROM event
        WHERE songId IN (SELECT songId FROM playlist_song_map WHERE playlistId = :playlistId)
    """)
    fun getPlaylistPlayCounts(playlistId: String): Flow<PlaylistPlayCountResult>
